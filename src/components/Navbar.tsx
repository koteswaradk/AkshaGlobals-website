  ...
  if (imgError) {
    return (
      <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <circle cx="20" cy="20" r="18" fill="none" stroke="#F97316" strokeWidth="1.5"/>
...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `px-3 py-2 rounded-full text-sm font-medium transition-colors duration-200 ${
      isActive
        ? 'text-white bg-[#1F2937] dark:bg-[#F97316]'
        : 'text-m3-on-surface-variant dark:text-m3-dark-on-surface-variant hover:text-[#F97316] dark:hover:text-[#F97316]'
    }`

  return (
    <nav className="bg-white dark:bg-m3-dark-surface-container shadow-m3-1 sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <span className="text-xl font-bold text-m3-on-surface dark:text-m3-dark-on-surface">
              Aksha<span className="text-[#F97316]"> Globals</span>
            </span>
          </a>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-white dark:bg-m3-dark-surface-container border-t border-m3-outline-variant dark:border-m3-dark-outline px-4 py-3 flex flex-col gap-1">
          {navLinks.map(link => (
            <NavLink
              key={link.to}
              to={link.to}
              className={({ isActive }) =>
                `block px-3 py-2 rounded-full text-sm font-medium transition-colors duration-200 ${
                  isActive
                    ? 'text-white bg-[#1F2937] dark:bg-[#F97316]'
                    : 'text-m3-on-surface-variant dark:text-m3-dark-on-surface-variant hover:text-[#F97316] dark:hover:text-[#F97316] hover:bg-m3-surface-container-high dark:hover:bg-m3-dark-surface-container-high'
                }`
              }
              end={link.to === '/'}
